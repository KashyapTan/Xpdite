...
import { formatModelLabel, isOpenRouterModel } from '../utils/modelDisplay';
                    ...
                    {enabledModels.map((model) => (
                      <option key={model} value={model}>
                        {isOpenRouterModel(model)
                          ? `OpenRouter • ${formatModelLabel(model)}`
                          : formatModelLabel(model)}
                      </option>
                    ))}
                  </select>
                  {isOpenRouterModel(selectedModel) && (
                    <span className="model-provider-badge" title="OpenRouter model">
                      <svg
