...
│   │   ├── tool_executor.py              # Shared MCP/inline execution for remote providers
│   │   └── types.py                      # ChatResult + provider-neutral types
│   └── providers/
│       ├── __init__.py
│       ├── ollama_provider.py            # AsyncClient streaming + Ollama tool loop integration
│       ├── cloud_provider.py             # Anthropic/OpenAI/Gemini/OpenRouter via LiteLLM
│       └── openai_codex_provider.py      # ChatGPT subscription turns via Codex app-server
│
├── mcp_integration/
│   ├── __init__.py
│   ├── core/
...
│   │   ├── codex_app_server.py           # Full-duplex Codex JSON-RPC/process transport
│   │   ├── openai_codex.py               # ChatGPT account/catalog facade
...
- **`infrastructure/runtime_capabilities.py`** — validates immutable packaged build metadata, probes native audio/embedding imports, and exposes product-level effective availability through `GET /api/runtime-capabilities`. Optional native failures disable only their feature. The `mac-intel-transcription` profile retains faster-whisper transcription and omits WhisperX, diarization, and bundled Sentence Transformers.
...
- **`services/integrations/codex_app_server.py` + `openai_codex.py`** — one long-lived, isolated Codex app-server owns ChatGPT OAuth, refresh, account state, rate limits, model entitlements, and turns. The transport is full-duplex JSON-RPC with generation-scoped pending calls and turn subscriptions. Codex is the only reader/writer of its canonical auth record; Xpdite never copies tokens into LiteLLM. The process and ephemeral threads use a connector-owned empty directory and a minimal environment. The app pins and bundles `@openai/codex`; `bun run test:codex-runtime` performs the real packaged handshake. macOS build copies are ad-hoc re-signed before the final Electron app signature so a revoked upstream Developer ID cannot make the helper disappear at launch.
...
| `settings` | `key`, `value` | Key-value store for all user preferences including `enabled_models`, `model_reasoning_efforts`, `system_prompt_template` |
...
### API-key cloud path (inline via LiteLLM)
Anthropic, OpenAI API, Gemini, and OpenRouter use `litellm.acompletion()`. API keys are passed directly as kwargs, tools are translated from OpenAI format, and tool calls execute through the shared `XpditeToolExecutor`. Multi-round streaming, reasoning, artifacts, cancellation, provider-error sanitization, and partial-response recovery remain unchanged.

Prompt caching is provider-native, not response memoization. OpenAI API sends a hashed `prompt_cache_key`; Anthropic and Claude-through-OpenRouter request ephemeral cache control. ChatGPT subscription usage/cache metadata comes only from app-server token-usage notifications. `source/llm/core/token_usage.py` normalizes provider usage into input, output, cached, and cache-write counters.

### ChatGPT subscription path (OpenAI Codex)
Models prefixed with `openai-codex/` route directly to `openai_codex_provider.py`. Each request creates an isolated ephemeral thread, sends the exact Xpdite `baseInstructions`, injects prior visible user/assistant history, registers only the retrieved Xpdite tools as dynamic tools, starts one turn, and interrupts it explicitly on cancellation. `environments: []` plus validated feature overrides remove environment-dependent Codex tools; any unexpected built-in execution item fails closed. App-server model metadata is authoritative and context capacity remains unknown when the protocol omits it.

Turns request Codex's readable reasoning summary (`summary: auto`). Stream only `item/reasoning/summaryTextDelta` (with the completed reasoning item's `summary` as a loss-recovery fallback) through `thinking_chunk` / `thinking_complete`; never expose `item/reasoning/textDelta`, which is private raw reasoning. Preserve thinking blocks in their stream position relative to tools and response text.

The Models page persists per-model effort overrides under `model_reasoning_efforts`, keyed by the fully-qualified `openai-codex/<picker-id>`. Main turns and ChatGPT-backed sub-agents pass the saved value to `turn/start` only when the live catalog reports it in `supportedReasoningEfforts`; stale values fall back safely.
...
| `GET` / `PUT` | `/api/models/reasoning-efforts` | Per-model ChatGPT reasoning-effort overrides |
...
- **OpenAI Codex / ChatGPT subscription**: Uses paginated account-scoped Codex `model/list`, excludes hidden models, preserves picker/default/reasoning/modality metadata, and prefixes picker IDs with `openai-codex/`. No context window is fabricated.
...
| `XPDITE_CODEX_BINARY` env var | — | Optional path to the pinned OpenAI Codex binary used for ChatGPT account, catalog, and inference |
