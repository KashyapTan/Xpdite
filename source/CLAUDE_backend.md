...
│       └── cloud_provider.py             # Unified Anthropic/OpenAI/Gemini/ChatGPT subscription via LiteLLM
│
├── mcp_integration/
│   ├── __init__.py
│   ├── core/
...
│   │   ├── openai_codex.py               # Codex app-server OAuth + model discovery
...
- **`thread_pool.py → run_in_thread`** is mandatory for anything that calls a synchronous SDK (e.g., `google-auth`, `Pillow`, screenshots). Calling them directly will block uvicorn's single event loop. The shared executor is recreated lazily after shutdown so same-process tests/restarts don't poison future calls. Note: Ollama now uses `AsyncClient` natively and no longer needs `run_in_thread`.
...
- **`services/skills_runtime/sub_agent.py`** — Sub-agent execution service. `execute_sub_agent()` is the entry point called by the `spawn_agent` tool interceptor (in both `cloud_provider.py` and `handlers.py`). Accepts `instruction`, `model_tier` (fast/smart/self), and `agent_name`. Resolves tier to a model via `_resolve_tier_model()` (checks DB setting `sub_agent_tier_<tier>`, falls back to current model). Tools are retrieved via `_get_sub_agent_tools()` which uses semantic retrieval minus `_EXCLUDED_TOOLS` (terminal tools + spawn_agent). Cloud and Ollama paths both stream progress/transcript updates through `sub_agent_stream`, with tool calls handled in multi-round loops. The LiteLLM cloud path mirrors the main chat recovery behavior: one retry for pre-first-chunk `MidStreamFallbackError`, recovery from `generated_content` when available, and the same sanitized provider-error surfacing used by the main cloud provider path. A global `_concurrency_semaphore` (cap 10) prevents overwhelming APIs/GPU. `execute_sub_agents_parallel()` checks if *any* call resolves to local Ollama — if so, all run sequentially; otherwise parallel with `asyncio.gather(return_exceptions=True)`. Ollama cloud tags `:cloud` and `-cloud` are treated as remote (parallel-safe). Broadcasts `tool_call` status messages with `server: "sub_agent"` for UI progress. Settings: `sub_agent_tier_fast` and `sub_agent_tier_smart` in the `settings` DB table, exposed via `GET/PUT /api/settings/sub-agents`.
- **`infrastructure/screenshot_runtime.py`** calls `SetProcessDpiAwarenessContext(-4)` (per-monitor V2) at import time via ctypes. Without this, Tkinter reports logical coordinates while the capture API uses physical pixels, causing misaligned region selection on scaled or multi-monitor Windows setups. Windows and Linux still use the Tk-based selector, but macOS interactive region capture now shells out to the native `screencapture -i -s` flow so AppKit UI is never created from the hotkey worker thread (`NSWindow` must be on the main thread there). The macOS path intentionally waits a short moment before launch so the `Control+.` modifier can clear and the frontend can hide itself. The listener now gates the hotkey at activation time, so region capture only launches while the app is in precision mode; once a capture has started, the scheduled attach path is forced through even if the global capture mode changes before the screenshot is added back to chat. `ScreenshotService` also emits a cancel callback when the user presses Escape or otherwise aborts region selection, and that callback rebroadcasts `screenshot_cancelled` so the frontend can reshow the hidden window on both macOS and Windows. Also exposes `copy_image_to_clipboard(image, dpi_scale=None)` (copies PIL Image to Windows clipboard via `CF_DIB` with retry loop for busy clipboard) and `copy_file_to_clipboard(filepath)` (uses PowerShell `Set-Clipboard -Path`). `ScreenshotService` has a `start_callback` field for when capture starts (used for window hiding) and a 1.5s debounce via `_last_trigger_time`. **Auto Mode:** when `app_state.auto_mode_enabled` is set, `on_activate` fires `auto_callback` (the hands-off pipeline) instead of a region capture, regardless of capture mode. `ScreenshotService` also registers a second, always-on global hotkey (`Control+,` / `Alt+,`) that fires `mini_toggle_callback` (its own 0.5s debounce via `_last_mini_trigger_time`). Both callbacks are provided by `services/media/auto_mode.py`, which bridges the background listener thread onto the event loop and broadcasts `auto_mode_trigger` / `toggle_mini_mode`. Auto Mode settings live in the `settings` table under `auto_mode_*` keys; `app_state.auto_mode_enabled` is the gate the listener reads and is restored from the DB at boot in `start_server()` before the listener starts.
...
- Precision screenshot capture (`Control+.` on macOS, `Alt+.` elsewhere) always routes to a concrete tab.
...
- **`services/integrations/openai_codex.py`** — `OpenAICodexService` manages the official Codex `app-server` over stdio for ChatGPT subscription auth only. Chat turns run through `cloud_provider.py` via LiteLLM's `chatgpt/<model>` provider, preserving Xpdite's system prompt and MCP tool loop. In development it stores ChatGPT/Codex auth outside the repo (under the OS app-data location) and writes a LiteLLM-compatible `CHATGPT_TOKEN_DIR` auth file; legacy `user_data/openai-codex/auth.json` is migrated without copying generated runtime temp files. Status/model-list calls do not start or probe the Codex auth helper; the helper binary is resolved lazily only when a user starts browser or device-code sign-in. The app declares `@openai/codex` directly so macOS/Windows/Linux native runtimes are bundled from project dependencies into `dist-codex-runtime`, and development can fall back to the npm wrapper (`node_modules/@openai/codex/bin/codex.js`) when the native binary path is unavailable.
...
|---|---|---|
| `submit_query` | `content`, `capture_mode`, `model`, `tab_id`, optional `auto_mode` (Auto Mode forces full-screen capture even when the tab has history) | `_handle_submit_query` |
...
| `auto_mode_get_settings` | — | `_handle_auto_mode_get_settings` |
| `auto_mode_update_settings` | `settings` (`enabled`/`prompt`/`pinned_model`/`keep_context`/`flash`) | `_handle_auto_mode_update_settings` |
...
| `screenshot_start` / `screenshot_added` / `screenshot_removed` / `screenshots_cleared` / `screenshot_cancelled` | Screenshot lifecycle |
...
| `token_usage` | After response completes; payload includes `prompt_eval_count`, `eval_count`, and provider-reported `cached_tokens` / `cache_write_tokens` when available |
...
| `auto_mode_settings` | Reply to `auto_mode_get_settings` / `auto_mode_update_settings` |
| `auto_mode_trigger` | Auto Mode hotkey fired — content `{prompt, model, keep_context, flash}`; the frontend opens a tab + auto-submits (global, no `tab_id`) |
| `toggle_mini_mode` | Mini-mode hotkey (`Control+,` / `Alt+,`) fired — frontend flips mini state |
...
5. **Prompt cache behavior** — local Ollama calls pass `keep_alive` alongside `num_ctx` to keep the model/KV state warm where Ollama can reuse it. Ollama does not report cached-token counts, so Xpdite records only its `prompt_eval_count` / `eval_count`.

### Cloud path (inline via LiteLLM)
All cloud providers (Anthropic, OpenAI, ChatGPT subscription, Gemini, OpenRouter) use a single unified implementation via `litellm.acompletion()`. `litellm.modify_params = True` is set globally — required for Anthropic thinking+tools to work correctly. API keys are passed directly as kwargs (not via `os.environ`) for thread safety; ChatGPT subscription models are the exception and omit `api_key` so LiteLLM reads `CHATGPT_TOKEN_DIR`. Tool definitions use OpenAI format for all providers — LiteLLM translates to native formats. When accumulated tool call deltas are detected after a streaming round ends, tools are executed and results fed back in a multi-round loop (up to `MAX_MCP_TOOL_ROUNDS`). The presence of `pending_tool_calls` is the trigger — not `finish_reason` — because providers like Gemini may use a non-standard finish reason (e.g. `"stop"` instead of `"tool_calls"`). After the tool budget is exhausted, one final summarisation round runs without tools so the model can synthesise its answer. Thinking/reasoning tokens are broadcast via `thinking_chunk`; thinking state (`thinking_tokens`, `thinking_complete_sent`) is **reset per round** so multi-round tool loops get fresh thinking broadcasts. Reasoning/thinking capability and `max_output_tokens` are both derived from a single `litellm.get_model_info()` call cached once per request — no redundant queries and no hardcoded token limits. If the model is not in litellm's registry, `max_tokens` is omitted (except for Anthropic, which requires it — a 16384 fallback is used). If supported, `reasoning_effort` (configurable via `REASONING_EFFORT` in `infrastructure/config.py`) is passed and LiteLLM translates it to each provider's native format (Anthropic → `thinking`, Gemini → `thinkingConfig`/`thinking_level`, OpenAI → native `reasoning_effort`). Mid-stream `litellm.exceptions.MidStreamFallbackError` failures are handled explicitly: if LiteLLM exposes `generated_content`, the missing suffix is merged back into the response stream; if the error happens before the first chunk, one retry is attempted before surfacing a backend error. Upstream provider exceptions are passed through a shared sanitizer/extractor before reaching the UI, so quota, billing, auth, and permission failures surface as provider-specific messages instead of a generic "temporarily unavailable" banner while still redacting keys/tokens from error text. Malformed tool call JSON is reported back to the model as a tool-result error so it can self-correct. Cancellation during tool execution properly exits both the inner tool loop and the outer streaming loop. On exceptions, partial accumulated text and tool calls are preserved (not discarded). Providers that omit `tool_call_id` (e.g. Gemini) receive synthetic fallback IDs. `_sanitize_tool_args(value)` scrubs sensitive keys (`api_key`, `token`, `secret`, `password`, `authorization`, `cookie`, `session`, `key`) from tool arguments before logging/broadcasting — replaces values with `"[REDACTED]"`.

Prompt caching is provider-native, not response memoization. OpenAI and ChatGPT subscription paths send a hashed `prompt_cache_key` with no raw prompt text. Anthropic and Claude-through-OpenRouter paths request ephemeral cache control. Gemini/OpenRouter automatic cache hits are parsed when LiteLLM surfaces them. `source/llm/core/token_usage.py` normalizes provider usage into `prompt_eval_count`, `eval_count`, optional `cached_tokens`, and optional `cache_write_tokens`; cached tokens remain metadata and are not subtracted from context-window totals.

### ChatGPT subscription path (OpenAI Codex)
Models prefixed with `openai-codex/` bypass API-key lookup and use the same Xpdite cloud tool loop as the other LiteLLM providers. `cloud_provider.py` maps them to LiteLLM's native `chatgpt/<model>` provider, configures `CHATGPT_TOKEN_DIR` from `services/integrations/openai_codex.py`, and omits `api_key` so LiteLLM reads the ChatGPT subscription OAuth tokens. Tool retrieval, MCP execution, sub-agent spawning, system prompt injection, artifacts, screenshots, cancellation, and conversation persistence all stay owned by Xpdite. `/api/openai/codex/*` endpoints use `codex app-server` only for browser/device-code OAuth; they do not run chat completions or launch Codex CLI execution.
...
| `GET` | `/api/openai/codex/status` | ChatGPT subscription auth status |
| `POST` | `/api/openai/codex/connect/browser` | Start Codex ChatGPT browser sign-in |
| `POST` | `/api/openai/codex/connect/device` | Start Codex ChatGPT device-code sign-in |
| `POST` | `/api/openai/codex/cancel` | Cancel in-flight Codex sign-in |
| `POST` | `/api/openai/codex/disconnect` | Log out Codex and clear enabled subscription models |
...
- **OpenAI Codex / ChatGPT subscription**: Uses Codex app-server `model/list`, prefixes returned models with `openai-codex/`, and sorts `gpt-5.4` first when present.
...
| `OLLAMA_KEEP_ALIVE` | env `OLLAMA_KEEP_ALIVE`, else `30m` | Passed as local Ollama chat `keep_alive` so the daemon can retain loaded model/KV state |
...
| `XPDITE_CODEX_BINARY` env var | — | Optional path to the OpenAI Codex binary used only as the ChatGPT subscription OAuth helper |
