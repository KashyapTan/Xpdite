...
## File Search
- Use the dedicated `glob` server's `glob_files` tool for file and directory discovery by pattern.
- Use the dedicated `grep` server's `grep_files` tool for searching inside file contents.
...
- Do NOT use `run_command` for `grep`, `rg`, `ag`, `find`, `ls`, `dir`, or shell glob expansion when the dedicated search tools can do the job.
...
- When `truncated: true`, narrow the query instead of retrying broadly:
  - Reduce `path` to a smaller subtree.
  - Add or tighten `file_glob`.
  - Use `type` when a standard file family is obvious.
