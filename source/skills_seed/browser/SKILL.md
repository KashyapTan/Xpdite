...
2. **Always read the output** — after every command, the CLI returns a page snapshot with the current URL, page title, and an accessibility tree of interactive elements labeled with **ref IDs** (e.g. `e1`, `e5`, `e12`). You MUST use these ref IDs for subsequent interactions.
3. **Snapshot after navigation** — after `open`, `goto`, `click` (that navigates), or any page change, the output contains a snapshot. Read it to find element refs. Run `playwright-cli snapshot` any time you need a fresh view.
4. **One command at a time** — run a single `playwright-cli` command per `run_command` call, read the output, then decide the next action. Never chain multiple commands.
5. **Always close** when done — `playwright-cli close` or `playwright-cli close-all`.

## Quick Start

Each line below is a separate `run_command` call:
```
playwright-cli open https://example.com --headed
# → Read snapshot output: e1 [link "More information..."], etc.

playwright-cli click e1
# → Read new snapshot after navigation
...
**Step 1:** Open the page and read the snapshot carefully.
```
playwright-cli open https://example.com --headed
```
Output will contain a snapshot with element refs. Find the input you need — look for `[searchbox]`, `[combobox]`, `[textbox]`, or `[input]` entries with a ref ID.
...
- If a snapshot doesn't show the element you expect, call `playwright-cli snapshot` again — the page may still be loading dynamically.

## Understanding Snapshots

After each command, the output looks like:
```
### Page
...
```
The **ref IDs** (e1, e2, …) are your handles for interacting with elements. Always read the snapshot to find the correct ref before clicking, filling, or interacting.
...
playwright-cli open https://example.com --headed  # open and navigate
...
```
playwright-cli -s=auth open https://app.example.com/login --headed
